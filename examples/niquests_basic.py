"""niquests + hedge-python: basic adaptive hedging example.

Uses :class:`HedgedNiquestsSession` as a drop-in replacement for
``niquests.AsyncSession``. ``GET`` / ``HEAD`` / ``OPTIONS`` are hedged; bodied
methods (``POST`` / ``PUT`` / ``DELETE``) pass through unchanged because their
payload cannot be safely replayed.

Run::

    uv run python examples/niquests_basic.py
"""
